...
public:
    ...
    ~VOX_HESS() { vector<const vector<VOX_FACTOR>*>().swap(plvec_voxels); }

    void get_center(const PLV(3) & vec_orig, PLV(3) & origin_points_) {
        size_t pt_size = vec_orig.size();
        for (size_t i = 0; i < pt_size; i++) origin_points_.emplace_back(vec_orig[i]);
        return;
    }
    ...
    void push_voxel(const vector<VOX_FACTOR>* sig_orig, const vector<PLV(3)>* vec_orig) {
        ...
        for (int i = 0; i < win_size; i++)
            if ((*sig_orig)[i].N != 0) process_size++;
...
public:
    ...
    bool judge_eigen() {
        VOX_FACTOR covMat;
        for (int i = 0; i < win_size; i++)
            if (sig_tran[i].N > 0) covMat += sig_tran[i];
        ...
        for (int i = 0; i < 8; i++)
            if (covMats[i].N > 10) {
                ...
                if (child_eva0 > ratios[0] * eva0 && child_eva0 < ratios[1] * eva0) num_qua++;
                num_all++;
            }
    ...
    void recut() {
        if (octo_state == UNKNOWN) {
            int point_size = 0;
            for (int i = 0; i < win_size; i++) point_size += sig_orig[i].N;
            ...
            if (judge_eigen()) {
                ...
                vector<VOX_FACTOR>().swap(sig_orig);
                vector<VOX_FACTOR>().swap(sig_tran);
                for (int i = 0; i < win_size; i++) cut_func(i);
            }
        }
    ...
    void tras_opt(VOX_HESS& vox_opt) {
        if (octo_state == PLANE)
            ...
            for (int i = 0; i < 8; i++)
                if (leaves[i] != nullptr) leaves[i]->tras_opt(vox_opt);
    }
    ...
    void tras_display(int layer = 0) {
        ...
        if (octo_state == PLANE) {
            ...
            layer++;
            for (int i = 0; i < 8; i++)
                if (leaves[i] != nullptr) leaves[i]->tras_display(layer);
        }
    }
};
...
public:
    ...
    double divide_thread(vector<IMUST>& x_stats, VOX_HESS& voxhess, vector<IMUST>& x_ab, Eigen::MatrixXd& Hess, Eigen::VectorXd& JacT) {
        ...
        for (int i = 0; i < tthd_num; i++)
            mthreads[i] = new std::thread(&VOX_HESS::acc_evaluate2, &voxhess, x_stats, part * i, part * (i + 1), std::ref(hessians[i]), std::ref(jacobins[i]),
                                          std::ref(resis[i]));
        ...
        for (int i = 0; i < 10; i++) {
            ...
            if ((fabs(residual1 - residual2) / residual1) < 0.05 || i == 9) {
                ...
                for (int j = 0; j < win_size - 1; j++)
                    for (int k = j + 1; k < win_size; k++) hess_vec.push_back(Hess.block<DVEL, DVEL>(DVEL * j, DVEL * k).diagonal().segment<DVEL>(0));
                break;
            }
#else
