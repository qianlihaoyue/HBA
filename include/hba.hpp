...
class LAYER {
public:
    int pose_size, layer_num, max_iter, part_length, left_size, left_h_size, j_upper, tail, thread_num, gap_num, last_win_size, left_gap_num;
    ...
    void init_storage(int total_layer_num_) {
        ...
        for (int i = 0; i < thread_num; i++) mem_costs.push_back(0);
    }
    ...
    void init_parameter(int pose_size_ = 0) {
        ...
        printf(
            "layer_num %d | thread_num %d | pose_size %d | max_iter %d | part_length %d | gap_num %d | last_win_size %d | "
            "left_gap_num %d | tail %d | left_size %d | left_h_size %d | j_upper %d | "
            "downsample_size %f | voxel_size %f | eigen_ratio %f | reject_ratio %f\n",
            layer_num, thread_num, pose_size, max_iter, part_length, gap_num, last_win_size, left_gap_num, tail, left_size, left_h_size, j_upper,
