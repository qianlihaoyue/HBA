...
double matrixAbsSum(Eigen::MatrixXd mat) {
    ...
    for (int i = 0; i < mat.rows(); i++)
        for (int j = 0; j < mat.cols(); j++) sum += fabs(mat(i, j));
    return sum;
}

double sigmoid_w(double r) { return 1.0 / (1 + exp(-r)); }
...
struct IMUST {
    ...
    IMUST(double _t, const Eigen::Matrix3d &_R, const Eigen::Vector3d &_p, const Eigen::Vector3d &_v, const Eigen::Vector3d &_bg, const Eigen::Vector3d &_ba,
